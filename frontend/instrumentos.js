...
async function request(path, init = {}) { const token = sessionStorage.getItem(tokenKey) || ""; const response = await fetch(`/api/instruments${path}`, { ...init, headers: { "Content-Type": "application/json", ...(token ? { Authorization: `Bearer ${token}` } : {}), ...(window.SharedModuleLock?.headers?.() || {}), ...(init.headers || {}) } }); const data = await response.json().catch(() => ({})); if (!response.ok) { const failure = new Error(data.error || "Não foi possível concluir a operação."); failure.status = response.status; throw failure; } return data; }
function showList() { window.SharedModuleLock?.release?.(); listPanel.classList.remove("is-hidden"); editorPanel.classList.add("is-hidden"); history.replaceState({}, "", "instrumentos.html"); window.scrollTo({ top: 0, behavior: "smooth" }); }
...
async function openNew() { try { await window.SharedModuleLock?.release?.(); renderEditor((await request("/new")).instrument); setUrl("novo"); } catch (failure) { errorBox.textContent = failure.message; } }
async function openExisting(id) { try { renderEditor((await request(`/${encodeURIComponent(id)}`)).instrument); await window.SharedModuleLock?.acquire?.("instrumentos", id); setUrl("editar", id); } catch (failure) { errorBox.textContent = failure.message; } }
...
form.addEventListener("submit", async (event) => { event.preventDefault(); errorBox.textContent = ""; const button = document.querySelector('button[type="submit"][form="instrumentForm"]'); if (button) button.disabled = true; try { const data = collectData(); const path = current?.instrumentId ? `/${encodeURIComponent(current.instrumentId)}` : "/"; const result = await request(path, { method: current?.instrumentId ? "PUT" : "POST", body: JSON.stringify({ instrument: data }) }); renderEditor(result.instrument); await window.SharedModuleLock?.acquire?.("instrumentos", result.instrument.instrumentId); setUrl("editar", result.instrument.instrumentId); await loadRecords(); saveStatus.textContent = "Salvo"; } catch (failure) { errorBox.textContent = failure.message; errorBox.scrollIntoView({ behavior: "smooth", block: "center" }); } finally { if (button) button.disabled = false; } });
...
async function boot() { if (!sessionStorage.getItem(tokenKey)) { window.location.href = "index.html"; return; } try { app.classList.remove("is-locked"); document.querySelector("#deleteInstrument").hidden = !manager(); await loadRecords(); const params = new URLSearchParams(window.location.search); if (params.get("modo") === "novo") await openNew(); if (params.get("modo") === "editar" && params.get("id")) await openExisting(params.get("id")); } catch (failure) { if ([401, 403].includes(failure.status)) window.location.href = "index.html"; else { list.innerHTML = `<div class="instrument-empty"><strong>Não foi possível carregar o módulo.</strong><span>${escapeHtml(failure.message)}</span></div>`; window.AppBoot?.error(failure.message, () => boot()); } } }
