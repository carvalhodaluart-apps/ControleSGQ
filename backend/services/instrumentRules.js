...
function normalizeInstrument(input = {}) {
  ...
  return {
    ...
    calibration: { recordNumber: text(calibration.recordNumber, 100), sendDate: date(calibration.sendDate), calibrationDate: date(calibration.calibrationDate), returnDate: date(calibration.returnDate), lab: text(calibration.lab, 160), certificateNumber: text(calibration.certificateNumber, 120), certificatePdf: calibration.certificatePdf && typeof calibration.certificatePdf === "object" ? { name: text(calibration.certificatePdf.name, 180), data: text(calibration.certificatePdf.data, 20000000) } : { name: "", data: "" }, patterns: text(calibration.patterns, 1800), traceability: text(calibration.traceability, 1800), conditionFound: CONDITIONS.includes(calibration.conditionFound) ? calibration.conditionFound : "", resultBeforeAdjustment: text(calibration.resultBeforeAdjustment, 1200), adjustmentPerformed: calibration.adjustmentPerformed === true, resultAfterAdjustment: text(calibration.resultAfterAdjustment, 1200), measurementUncertainty: text(calibration.measurementUncertainty, 180), errorFound: text(calibration.errorFound, 180), acceptanceCriteria: text(calibration.acceptanceCriteria, 1200), finalResult: FINAL_RESULTS.includes(calibration.finalResult) ? calibration.finalResult : "", nextCalibration: date(calibration.nextCalibration), analysisResponsible: text(calibration.analysisResponsible, 120), releaseDate: date(calibration.releaseDate) },
