...
async function assertSharedLock(req, id) {
  if (sharedStorage.isConfigured()) await sharedStorage.assertModuleLock("instrumentos", id, getRequestUser(req), req.get("X-Module-Lock"));
}
...
router.post("/:id/lock", requireProcedureEditor, async (req, res) => { try { res.json(await sharedStorage.acquireModuleLock("instrumentos", req.params.id, getRequestUser(req))); } catch (error) { handleError(res, error); } });
router.post("/:id/lock/heartbeat", requireProcedureEditor, async (req, res) => { try { res.json(await sharedStorage.refreshModuleLock("instrumentos", req.params.id, getRequestUser(req), req.get("X-Module-Lock"))); } catch (error) { handleError(res, error); } });
router.delete("/:id/lock", requireProcedureEditor, async (req, res) => { try { res.json(await sharedStorage.releaseModuleLock("instrumentos", req.params.id, getRequestUser(req), req.get("X-Module-Lock"))); } catch (error) { handleError(res, error); } });
...
router.put("/:id", requireProcedureEditor, async (req, res) => { try { await assertSharedLock(req, req.params.id); const instrument = await updateInstrument(req.params.id, validateInstrument({ ...(req.body?.instrument || req.body), instrumentId: req.params.id }), getRequestUser(req)); await recordAudit({ procedureId: instrument.instrumentId, action: "instrument-updated", user: getRequestUser(req), details: { documentCode: instrument.documentCode, situation: instrument.situation } }); res.json({ instrument }); } catch (error) { handleError(res, error); } });
router.delete("/:id", requireManager, async (req, res) => { try { await assertSharedLock(req, req.params.id); await deleteInstrument(req.params.id); await recordAudit({ procedureId: req.params.id, action: "instrument-deleted", user: getRequestUser(req) }); res.json({ ok: true }); } catch (error) { handleError(res, error); } });
